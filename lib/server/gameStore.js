...
function createStore() {
  ...
  // Lockless read + conditional write: loads the room without acquiring the lock,
  // runs the handler, and only saves if the handler returns { dirty: true, result }.
  async function withRoomReadTransaction(code, handler) {
    const normalizedCode = code?.toUpperCase();
    const room = await loadRoom(normalizedCode);
    const { dirty, result } = await handler(room);
    if (dirty && room) {
      // Acquire lock only when we actually need to write.
      const lock = await acquireRoomLock(normalizedCode);
      try {
        await saveRoom(room);
      } finally {
  ...
  return {
    ...
    async getRoomSnapshot(code) {
      return withRoomReadTransaction(code, async (room) => {
        if (!room) return { dirty: false, result: null };
        const now = Date.now();
        let dirty = false;
        for (const player of room.players.values()) {
          const snap = { state: player.engagementState, earnings: player.earnings, trip: player.currentTrip?.tripId, req: player.pendingRequest?.requestId };
          syncPlayer(room, player, now);
          if (player.engagementState !== snap.state || player.earnings !== snap.earnings || player.currentTrip?.tripId !== snap.trip || player.pendingRequest?.requestId !== snap.req) dirty = true;
        }
        return { dirty, result: buildRoomSnapshot(room) };
      });
    },
    ...
    async getPlayerState(code, playerId, playerToken) {
      return withRoomReadTransaction(code, async (room) => {
        ...
        for (const currentPlayer of room.players.values()) {
          const snap = { state: currentPlayer.engagementState, earnings: currentPlayer.earnings, trip: currentPlayer.currentTrip?.tripId, req: currentPlayer.pendingRequest?.requestId };
          syncPlayer(room, currentPlayer, now);
          if (currentPlayer.engagementState !== snap.state || currentPlayer.earnings !== snap.earnings || currentPlayer.currentTrip?.tripId !== snap.trip || currentPlayer.pendingRequest?.requestId !== snap.req) dirty = true;
        }
        const player = room.players.get(playerId);
        if (!player) return { dirty: false, result: null };
        if (player.playerToken !== playerToken) return { dirty: false, result: { error: 'Unauthorized' } };
        return { dirty, result: buildPlayerState(room, player) };
      });
    },
      ...
      return withRoomTransaction(code, async (room) => {
        ...
        case 'maintenance_fee': {
          if (activePlayers.length > 0) {
            ...
            const fee = roundCurrency(randomBetween(150, 400));
            target.earnings = roundCurrency(target.earnings - fee);
            target.strainLevel = Math.min(100, target.strainLevel + Math.floor(randomBetween(10, 18)));
            target.acceptanceRate = Math.max(0, target.acceptanceRate - Math.floor(randomBetween(2, 5)));
            target.eventCursor += 1;
            target.latestMaintenanceFee = {
              eventId: `maintenance-${target.eventCursor}`,
              amount: fee,
              reason: 'Vehicle repair: unscheduled maintenance',
              issuedAt: now,
            };
            markMechanic(target, 'maintenance_fee_shock');
          }
          room.currentEvent = { type: eventType, expiresAt: now + 1500 };
          break;
        }
