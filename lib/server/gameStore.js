...
function createStore() {
  ...
  return {
    ...
    async getRoomSnapshot(code) {
      const room = await this.syncRoom(code, { persist: false });
      if (!room) return null;
      return {
        code: room.code,
        phase: room.phase,
    ...
    async getPlayerState(code, playerId, playerToken) {
      const room = await this.syncRoom(code);
    ...
    async startGame(code, hostToken) {
      ...
      await saveRoom(room);
      return { success: true, snapshot: await this.getRoomSnapshot(code) };
    },
    ...
    async triggerQuestion(code, hostToken) {
      ...
      Array.from(room.players.values())
        ...
        .forEach((player) => {
          player.activeQuiz = {
            quizId: `${question.id}-${Date.now()}-${player.playerId}`,
      ...
      await saveRoom(room);
      return { success: true, snapshot: await this.getRoomSnapshot(code), question };
    },
    ...
    async answerQuestion(code, playerId, playerToken, selectedIndex) {
      ...
      return {
        ...
        state: await this.getPlayerState(code, playerId, playerToken),
      };
    },
    ...
    async respondQuest(code, playerId, playerToken, accepted) {
      ...
      player.pendingQuestOffer = null;
      await saveRoom(room);
      return { success: true, state: await this.getPlayerState(code, playerId, playerToken) };
    },
