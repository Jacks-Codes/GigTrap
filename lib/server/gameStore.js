...
const PROMO_MESSAGES = [
  'Try GigTrap Eats - deliver food between rides!',
  'Earn an extra $5 - sign up for GigTrap Direct!',
  'Free gas card - complete 10 more rides today!',
  'Unlock cashback rewards - tap to learn more!',
  'GigTrap Pro: priority rides for $9.99/mo. Try free for 7 days!',
  'Refer a friend, earn $50 - share your code now!',
  'New: GigTrap Rentals - drive a newer car for less. Learn more.',
  'Complete 5 rides by midnight for a $3 bonus!',
];

const PROMO_INTERVAL_MIN_MS = 120000;
const PROMO_INTERVAL_MAX_MS = 180000;
...
function calculateTier(ridesCompleted, acceptanceRate) {
  for (let i = TIER_LEVELS.length - 1; i >= 0; i--) {
    const tier = TIER_LEVELS[i];
    if (ridesCompleted >= tier.minRides && acceptanceRate >= tier.minAcceptance) {
      return i;
    }
  }
  return 0;
...
function shiftPlayerDeadlines(player, deltaMs) {
  ...
  player.nextPromoAt = shiftTimestamp(player.nextPromoAt, deltaMs);
}
...
function syncPlayer(room, player, now = Date.now()) {
  ...
  if (!player.activePromo && !player.isDeactivated) {
    if (!player.nextPromoAt) {
      player.nextPromoAt = now + Math.floor(randomBetween(PROMO_INTERVAL_MIN_MS, PROMO_INTERVAL_MAX_MS));
    }
    if (now >= player.nextPromoAt) {
      player.activePromo = {
        promoId: `promo-${player.playerId}-${now}`,
        message: PROMO_MESSAGES[Math.floor(Math.random() * PROMO_MESSAGES.length)],
      };
      player.nextPromoAt = null;
      markMechanic(player, 'promo_spam');
    }
  }

  const newTierIndex = calculateTier(player.ridesCompleted, player.acceptanceRate);
  const oldTierIndex = player.tierIndex || 0;
  if (newTierIndex < oldTierIndex) {
    player.strainLevel = Math.min(100, player.strainLevel + 10);
    markMechanic(player, 'tier_pressure');
  }
  player.tierIndex = newTierIndex;
  player.tierName = TIER_LEVELS[newTierIndex].name;
  player.tierColor = TIER_LEVELS[newTierIndex].color;
}
...
function sanitizePlayer(player) {
  return {
    ...
    promosDismissed: player.promosDismissed || 0,
    ...
    latestMaintenanceFee: player.latestMaintenanceFee,
...
function createStore() {
  ...
  return {
    ...
    async triggerEvent(code, hostToken, eventType) {
      return withRoomTransaction(code, async (room) => {
        ...
        case 'maintenance_fee': {
          ...
          const amount = parseFloat(Math.floor(randomBetween(150, 400)).toFixed(2));
          ...
          target.latestMaintenanceFee = {
            eventId: `maint-${target.playerId}-${target.eventCursor}`,
            reason: issue,
            amount,
          };
          target.pendingMaintenanceFee = null;
    ...
    async dismissPromo(code, playerId, playerToken) {
      return withRoomTransaction(code, async (room) => {
        if (!room) return { error: 'Player not found' };
        const player = room.players.get(playerId);
        if (!player) return { error: 'Player not found' };
        if (player.playerToken !== playerToken) return { error: 'Unauthorized' };
        player.activePromo = null;
        player.promosDismissed = (player.promosDismissed || 0) + 1;
        player.nextPromoAt = Date.now() + Math.floor(randomBetween(PROMO_INTERVAL_MIN_MS, PROMO_INTERVAL_MAX_MS));
        return { success: true, state: buildPlayerState(room, player) };
      });
    },
